...
function excel(data, event) {
  ...
  const durationRow = worksheet.addRow(['', `총 ${data.length}명`, '', '', '일시:', duration]);
  durationRow.getCell(5).alignment = { vertical: 'middle', horizontal: 'right' };
  const durationCell = durationRow.getCell(6);
  durationCell.alignment = { vertical: 'middle', horizontal: 'right' };

  // Set column headers
  const columnHeaderRow = worksheet.addRow(['', '순번', '학부', '학번', '이름', '태깅시간']);
  ...
  data.forEach((item, index) => {
    const rowData = ['', index + 1, item.department, item.studentNum, item.name, fDateString(item.taggedAt)]; // Add an empty cell on the left side
  ...
  worksheet.getColumn(2).width = 8; // 순번
  worksheet.getColumn(3).width = 20; // 학부
  worksheet.getColumn(4).width = 12; // 학번
  worksheet.getColumn(5).width = 12; // 이름
  worksheet.getColumn(6).width = 28; // 태깅시간
